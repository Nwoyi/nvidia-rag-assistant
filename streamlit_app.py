...
def get_ai_client(api_key):
    ...
    # This avoids redundant object instantiation on every Streamlit script rerun,
    # providing a significant speedup for UI responsiveness.
...
def generate_answer(query, _search_results):
    # Bolt ⚡: Optimized generate_answer for performance:
    # 1. Used _search_results to skip Streamlit's expensive hashing of complex Qdrant objects.
    # 2. Refactored context building to use "".join() for O(N) string construction.
    # 3. Returns only the answer string to minimize the cache memory footprint.
