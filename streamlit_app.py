...
def get_ai_client(api_key):
    # Bolt ⚡: Caching the AI client. This prevents redundant client initialization on every rerun.
...
def generate_answer(query, _search_results):
    # Bolt ⚡: Optimizing cache performance.
    # 1. Added a TTL of 1 hour to manage memory and prevent stale results.
    # 2. Used a leading underscore for the '_search_results' parameter. This tells Streamlit
    #    to skip hashing the large, non-serializable Qdrant response object,
    #    reducing cache lookup latency.
    # 3. Using a list and "".join() for more efficient string construction of the context.
    # 4. Returning only the final answer string to minimize the cache memory footprint.
        ...
        with st.spinner("Searching knowledge base..."):
            try:
                ...
                with st.expander("📚 View Sources"):
                    # Bolt ⚡: Accessing search results directly from 'search_hits'
                    # as 'generate_answer' now returns only the answer string.
