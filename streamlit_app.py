...
def generate_answer(query, _search_results):
    # Bolt ⚡: Caching the generation step and skipping hashing for search_results
    # because the QueryResponse object is large and non-serializable.
    """Feeds search results into the LLM to get a human-like answer."""
    context_parts = []
    for i, hit in enumerate(_search_results.points):
        # Bolt ⚡: Using list + join for faster string construction.
