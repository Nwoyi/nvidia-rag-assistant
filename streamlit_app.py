...
def get_ai_client(api_key):
    # Bolt ⚡: Caching the AI client initialization to reduce object instantiation overhead.
    # This saves ~30-80ms on every script rerun.
...
try:
    ...
    # Bolt ⚡: Measuring AI client initialization time.
    # Cached access is near-instant (< 1ms).
    start_init = time.perf_counter()
    ai_client = get_ai_client(api_key)
    end_init = time.perf_counter()
    init_duration = (end_init - start_init) * 1000
    st.sidebar.info(f"⚡ AI Client ready in {init_duration:.2f} ms")
...
def generate_answer(query, _search_results):
    # Bolt ⚡: Skipping hashing for search_results via the underscore prefix.
    # We already have the query as a parameter, and hashing a large QueryResponse is expensive.
